...
ZEGO Conversational Agent Service 是面向实时 AI Agent 的可部署对话编排服务。
...
ZEGO AI Agent 服务端负责实时媒体和实例层：RTC 音频、ASR、TTS、打断和 AgentInstance 生命周期。ZEGO Conversational Agent Service 负责文字侧智能层：ZEGO LLM callback、workspace/mode/skill 逻辑、动态会话上下文、action 信令和结构化日志。
...
客户 Web 或移动 Web 应用
  ...
  -> ZEGO AI Agent 服务端调用 ZEGO Conversational Agent Service 获取 LLM 文本
  -> ZEGO Conversational Agent Service 返回 ZEGO TTS 可播报文本，并通过 HTTP、MCP 或 adapter 连接客户 Skill 系统
```

## Runtime 边界

- **ZEGO Conversational Agent Gateway 服务** 接收 ZEGO AI Agent LLM 回调，运行 workspace/mode/skill/automation 逻辑，输出 ZEGO TTS 可播报文本，并交付 mode/action/perf/agent 事件。
...
- 其余 `/voice*` -> ZEGO Conversational Agent Gateway
