...
`pulse-conversation-agent` 是客户自部署的实时 ZEGO Conversational Agent Gateway，负责实时 AI Agent 的文本侧智能编排。
...
| ZEGO Conversational Agent Gateway | LLM callback、workspace runtime、阶段指引、Action/Skill 编排、动态上下文、观测。 |
...
ZEGO Conversational Agent 是编译制品的对外 preview 入口。源码不包含在本 preview 中，AI 编程工具相关材料由独立的 developer-assistant 仓库维护。

| 入口 | 用途 |
| --- | --- |
| ZEGO Conversational Agent 仓库 | 评估文档、release manifest、checksum 和编译后的 preview artifact。 |
